...
func checkVersion(cmd *cmdutils.Cmd, ctl *eks.ClusterProvider, meta *api.ClusterMeta) error {
	...
	case "default":
		...
		logger.Infof("will use default version (%s) for new nodegroup(s)", meta.Version)
	case "latest":
		meta.Version = api.LatestVersion
		logger.Infof("will use latest version (%s) for new nodegroup(s)", meta.Version)
	default:
	...
	if v := ctl.ControlPlaneVersion(); v == "" {
		...
		meta.Version = v
		logger.Infof("will use version %s for new nodegroup(s) based on control plane version", meta.Version)
	} else if meta.Version != v {
		hint := "--version=auto"
		if cmd.ClusterConfigFile != "" {
			hint = "metadata.version: auto"
		}
		logger.Warnf("will use version %s for new nodegroup(s), while control plane version is %s; to automatically inherit the version use %q", meta.Version, v, hint)
	}
...
func showDevicePluginMessageForNodeGroup(nodeGroup *api.NodeGroup, installNeuronPlugin bool) {
	if api.HasInstanceType(nodeGroup, utils.IsInferentiaInstanceType) {
		if installNeuronPlugin {
			logger.Infof("as you are using the EKS-Optimized Accelerated AMI with an inf1 instance type, the AWS Neuron Kubernetes device plugin was automatically installed.")
			logger.Infof("\t to skip installing it, use --install-neuron-plugin=false.")
		} else {
			// if neuron instance type, give instructions
			logger.Infof("as you are using the EKS-Optimized Accelerated AMI with an inf1 instance type, you will need to install the AWS Neuron Kubernetes device plugin.")
			logger.Infof("\t see the following page for instructions: https://github.com/aws/aws-neuron-sdk/blob/master/docs/neuron-container-tools/tutorial-k8s.md")
		}
	} else if api.HasInstanceType(nodeGroup, utils.IsGPUInstanceType) {
		// if GPU instance type, give instructions
		logger.Infof("as you are using a GPU optimized instance type you will need to install NVIDIA Kubernetes device plugin.")
		logger.Infof("\t see the following page for instructions: https://github.com/NVIDIA/k8s-device-plugin")
	}
}
